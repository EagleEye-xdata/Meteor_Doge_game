...
def draw(player, elapsed_time, star_list, score, highscore, shield_rect, shield_active, speed_rect, speed_active, lives, paused):
    WIN.blit(BG, (0, 0))
...
def main_menu():
    ...
    while run:
        ...
        instr4 = FONT.render("Avoid yellow objects! You have 3 lives.", 1, "red")
...
def main():
    ...
    while run:
        ...
        if not paused and current_time - last_star_add_time >= star_add_increment:
            for _ in range(5):
                star_x = random.randint(0, WIDTH - STAR_WIDTH)
                star = pygame.Rect(star_x, -100, STAR_WIDTH, STAR_HEIGHT)
        ...
        if paused:
            draw(player, elapsed_time, star_list, score, highscore, shield_rect, shield_active, speed_rect, speed_active, lives, paused)
            continue
